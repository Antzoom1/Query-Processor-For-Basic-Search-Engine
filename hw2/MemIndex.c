...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (!(HashTable_Find(index, key, &mi_kv))) {
    ...
    Verify333(wp != NULL);
    wp->word = word;
    // allocates a new hashtable for the WordPostings' docID->postings mapping
    wp->postings = HashTable_Allocate(16);
    Verify333(wp->postings != NULL);
    // sets and inserts the the new WordPostings into the inverted index
    mi_kv.key = key;
    mi_kv.value = wp;
...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  // if the query word is found in the inverted index
  wp = (WordPostings*) kv.value;
  ret_list = LinkedList_Allocate();
  // creates a new iterator for the postings hashtable
  HTIterator *htIterator = HTIterator_Allocate(wp->postings);
  while (HTIterator_IsValid(htIterator)) {
    // gets the next key-value pair from the postings hashtable
    HTIterator_Get(htIterator, &kv);
    // allocates a SearchResult structure
    ...
    // iterates to the next key-value pair
    HTIterator_Next(htIterator);
  }
  HTIterator_Free(htIterator);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    // if the query word is not found in the inverted index
    if (!HashTable_Find(index, key, &kv)) {
    ...
    for (j = 0; j < num_docs; j++) {
      // allocates a SearchResult structure and gets the next element
      SearchResult *res;
