...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  res = HashTable_Find(table->name_to_id, key, &old_kv);
  if (res) {
  ...
  *doc_id = table->max_id;
  ...
  res = HashTable_Insert(table->id_to_name, kv, &old_kv);
  // We should never be overwriting an existing key here
  Verify333(!res);
  ...
  kv.key = key;
  kv.value = doc_id;
  res = HashTable_Insert(table->name_to_id, kv, &old_kv);
  Verify333(res == 0);
  return *doc_id;
}
...
char* DocTable_GetDocName(DocTable* table, DocID_t doc_id) {
  ...
  int res = HashTable_Find(table->id_to_name, (HTKey_t) doc_id, &kv);
  if (res == 1) {
    return kv.value;
  } else {
    return NULL;
