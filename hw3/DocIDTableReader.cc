  ...
  for (IndexFileOffset_t& curr_element : elements) {
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      for (int i = 0; i < curr_header.num_positions; i++) {
        ...
        // add to the list
        positions.push_back(curr_position.position);
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    // the index file.
    Verify333(fseek(file_, offset_ + sizeof(BucketListHeader) + i
      * sizeof(BucketRecord), SEEK_SET) == 0);

    // STEP 5.
    // Read in the chain length and bucket position fields from
    // the bucket_rec.
    BucketRecord bucket_rec;
    // checks if bucket size is within boundaries
    ...
    for (int j = 0; j < bucket_rec.chain_num_elements; j++) {
      ...
      // checks if element size is within boundaries
      Verify333(fread(&element_pos,
       sizeof(ElementPositionRecord), 1, file_) == 1);
      // convertts to host format
      ...
      // converts element to host format
