...
HashTableReader::HashTableReader(FILE* f, IndexFileOffset_t offset)
  ...
  // converts header to host format
...
HashTableReader::LookupElementPositions(HTKey_t hash_key) const {
  ...
  for (int i = 0; i < bucket_rec.chain_num_elements; i++) {
    ElementPositionRecord element_record;
    // checks if the file size is correct
    Verify333(fread(&element_record,
    sizeof(ElementPositionRecord), 1, file_) == 1);
    // converts the element record to host format
    element_record.ToHostFormat();
    ret_val.push_back(element_record.position);
  }
  // Return the list.
  return ret_val;
}
