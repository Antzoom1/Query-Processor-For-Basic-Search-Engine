...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  CRC32 crc;
  // Move to the start of the doctable
  ...
  int left_to_read = memidx_bytes + doctable_bytes;
  // Loop through bytes of doctable and memindex to calculate CRC
  while (left_to_read) {
    size_t amt_read = fread(buf, sizeof(uint8_t), kBufSize, f);
    // If we read less than kBufSize, we should be at the end of the file.
    if (amt_read != kBufSize) {
      if (!feof(f)) {
        return kFailedWrite;
      }
    }
    for (size_t i = 0; i < amt_read; i++) {
      crc.FoldByteIntoCRC(buf[i]);
    }
    left_to_read -= amt_read;
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int bucket_size = LinkedList_NumElements(ht->buckets[i]);
    int record_bytes = WriteHTBucketRecord(f, record_pos, bucket_size,
                                           bucket_pos);
    if (record_bytes == kFailedWrite) {
      return kFailedWrite;
    }
    record_pos += record_bytes;
    // Write the bucket
    int bucket_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    if (fwrite(&record, sizeof(ElementPositionRecord), 1, f) != 1) {
      // Free iterator if failed before returning
      LLIterator_Free(it);
      return kFailedWrite;
    }
    ...
    // Call the write function to write the element.
    int element_bytes = fn(f, element_pos, payload);
    ...
    element_pos += element_bytes;  // you probably want to change this
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  // file_name_bytes does not include the null terminator
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  // word_bytes is the length of the word, not including the null
