...
static HttpResponse ProcessFileRequest(const string& uri,
  ...
  // We already know that the URI starts with "/static/"
  string file_name = uri.substr(8);

  string file_content;
  FileReader fr(base_dir, file_name);
  if (!(fr.ReadFile(&file_content))) {
    // Couldn't find the file or isn't path safe
    // Return a 404 error
  ...
  if (file_name.substr(file_name.length() - 5) == ".html" ||
      file_name.substr(file_name.length() - 4) == ".htm") {
    ret.set_content_type("text/html");
  } else if (file_name.substr(file_name.length() - 5) == ".jpeg" ||
              file_name.substr(file_name.length() - 4) == ".jpg") {
    ret.set_content_type("image/jpeg");
  } else if (file_name.substr(file_name.length() - 4) == ".png") {
    ret.set_content_type("image/png");
  } else if (file_name.substr(file_name.length() - 3) == ".js") {
    ret.set_content_type("text/javascript");
  } else if (file_name.substr(file_name.length() - 4) == ".css") {
    ret.set_content_type("text/css");
  } else if (file_name.substr(file_name.length() - 4) == ".xml") {
...
static HttpResponse ProcessQueryRequest(const string& uri,
  ...
  URLParser parser;
  parser.Parse(uri);
  if (parser.path() != "/query") {
    return ret;
  }

  // Get the query
  map<string, string> args = parser.args();
  if (args.size() == 0 || args.begin()->first != "terms") {
    // Query is not formatted correctly
    ret.AppendToBody("<p>Query is not formatted correctly</p>");
    return ret;
  }
  string query = args.begin()->second;
  boost::algorithm::to_lower(query);
  // Return no serach results if the query is empty
  if (query.size() == 0) {
    return ret;
  }
  ...
  // EscapeHtml the query before displaying it
  query = EscapeHtml(query);

  // Display the results
  if (results.size() == 0) {
    ret.AppendToBody("<p>No results found for <strong>"
                        + query + "</strong></p>");
    return ret;
  }
  ...
  searchResults << "<p>" << results.size() << " results found for <strong>"
                << query << "</strong></p>";

  // Display each search result as a list item
  searchResults << "<ul>";
  for (auto result : results) {
    searchResults << "<li><a href=\"";
    // Check if result is a url or a file
    if (result.document_name.substr(0, 7) == "http://") {
      searchResults << result.document_name << "\">";
    } else {
      searchResults << "/static/" << result.document_name << "\">";
    }
    searchResults << result.document_name << "</a> ["
                  << result.rank << "]</li></br>";
  }
  searchResults << "</ul>";
  ret.AppendToBody(searchResults.str());
