...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  char* abs_test = realpath(test_file.c_str(), nullptr);
  if (abs_test == nullptr) {
    // Error getting absolute path
    return false;
  }

  char* abs_root = realpath(root_dir.c_str(), nullptr);
  if (abs_root == nullptr) {
    // Error getting absolute path
    free(abs_test);
    return false;
  ...
  string abs_test_str(abs_test);
  free(abs_test);
  string abs_root_str = string(abs_root) + "/";
  free(abs_root);

  // Check if abs_str starts with root_dir
  if (abs_test_str.find(abs_root_str) == 0) {
    return true;
  }
