...
bool FileReader::ReadFile(string* const contents) {
  ...
  // STEP 1:
  // Check that the file path is safe to read from
  if (!IsPathSafe(basedir_, fname_)) {
    return false;
  }
  int file_size;
  char *file_contents = ReadFileToString(full_file.c_str(), &file_size);
  // If there is no content in the file, return false
  if (file_contents == nullptr) {
    return false;
  }
  // Otherwise copy the file contents to the string output parameter
