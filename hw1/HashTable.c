...
// Helper method that finds a certain key in the linked list and keeps the
// iterator pointing at that node. Also puts the keyvalue pair in kv if found.
// Returns true if the key is found, false if not.
bool FindInChain(LLIterator *iter, HTKey_t key, HTKeyValue_t *kv) {
  HTKeyValue_t *currkeyvalue;
  while (LLIterator_IsValid(iter)) {
    LLIterator_Get(iter, (LLPayload_t *) &currkeyvalue);
    if (key == currkeyvalue->key) {
      kv->key = currkeyvalue->key;
      kv->value = currkeyvalue->value;
      return true;
    }
...
// Free the payload which is just a pointer to a HTKeyValue_t
...
bool HashTable_Insert(HashTable *table,
  ...
  if (FindInChain(iter, newkeyvalue.key, oldkeyvalue)) {
    // Key already exists, so we need to remove the existing value
    // from the chain first before adding in the new keyvalue pair
    LLIterator_Remove(iter, FreePayload);
    ret = true;
  } else {
    // Key doesn't exist, so we're adding a new element to the chain
    table->num_elements++;
  }

  // Need to make dynamic memory for *newkeyvalue or else it will be
  // destroyed when this function ends
  HTKeyValue_t *newkeyvalueptr = (HTKeyValue_t *) malloc(sizeof(HTKeyValue_t));
  *newkeyvalueptr = newkeyvalue;

  // Add the new keyvalue pair to the chain
  LinkedList_Append(chain, (LLPayload_t) newkeyvalueptr);

  // Free the iterator and return
...
bool HashTable_Find(HashTable *table,
  ...
  LLIterator* iter = LLIterator_Allocate(chain);
  // FindInChain will put the keyvalue pair in keyvalue if found
  if (FindInChain(iter, key, keyvalue)) {
    // Key was found, so we need to return true
    LLIterator_Free(iter);
    return true;
  }
  // Key was not found, so we need to return false
  LLIterator_Free(iter);
  return false;
}
...
bool HashTable_Remove(HashTable *table,
  ...
  // FindInChain will put the keyvalue pair in keyvalue if found
  if (FindInChain(iter, key, keyvalue)) {
    // Key was found, so we need to remove it from the chain
    LLIterator_Remove(iter, FreePayload);
    table->num_elements--;
    LLIterator_Free(iter);
    return true;
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // checks if the bucket iterator is valid
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // check if the iterator is valid before moving to the next element
  ...
  if (!(LLIterator_IsValid(iter->bucket_it))) {
    ...
    for (int i = iter->bucket_idx + 1; i < iter->ht->num_buckets; i++) {
      if (LinkedList_NumElements(iter->ht->buckets[i]) > 0) {
        // if the next bucket is valid, set the bucket
        // iterator to the next bucket
        // and allocated space for it
        iter->bucket_idx = i;
        iter->bucket_it = LLIterator_Allocate(iter->ht->buckets[i]);
        return true;
      }
    }
    // if the next bucket is not valid, set the bucket iterator to NULL
    iter->bucket_idx = INVALID_IDX;
    iter->bucket_it = NULL;
    return false;
  }
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  // check if the iterator is valid before getting the element
  if (!(HTIterator_IsValid(iter))) {
    return false;
  }
  // if the iterator is valid, get the element
  HTKeyValue_t* ptr;
  LLIterator_Get(iter->bucket_it, (LLPayload_t *) &ptr);
  // set the keyvalue to the element and return true
